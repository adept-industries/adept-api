...
test -n "${GRAFANA_CLOUD_LOGS_USER:-}" || { echo "Set GRAFANA_CLOUD_LOGS_USER in .env.monitoring" >&2; exit 1; }
...
case "${GRAFANA_CLOUD_LOGS_URL:-}" in
  https://*) ;;
  *) echo "Set an HTTPS GRAFANA_CLOUD_LOGS_URL in .env.monitoring" >&2; exit 1 ;;
esac
